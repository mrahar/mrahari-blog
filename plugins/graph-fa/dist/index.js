...
(function () {
  function getSlugFromUrl() {
    ...
    // FIX: window.location.pathname percent-encodes non-ASCII (e.g. Persian)
    // slugs, but contentIndex keys are decoded. Without decoding, the current
    // node never matches any edge and shows up isolated with an encoded label.
    try {
  ...
  function initGraph() {
    ...
    function addToVisited(slug) {
      // Decode so Persian slugs coming from an encoded nav URL match node ids.
      try {
        slug = decodeURIComponent(slug);
      } catch (e) {
        /* keep raw slug */
...
export { Graph_default as GraphFa };
